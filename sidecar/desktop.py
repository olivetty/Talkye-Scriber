...
rec_start_time = 0.0
busy = False
# Runtime-mutable trigger key (updated via server API)
TRIGGER_KEY = os.getenv("DICTATE_KEY", "KEY_RIGHTCTRL")
# Sound theme: subtle | mechanical | silent
SOUND_THEME = os.getenv("DICTATE_SOUND_THEME", "subtle")
WAKEWORD_THRESHOLD = float(os.getenv("DICTATE_WAKEWORD_THRESHOLD", "0.02"))
...
def _generate_sounds():
    ...
    for theme_name, sounds in themes.items():
        for name, effect in sounds.items():
            path = os.path.join(SOUNDDIR, f"{theme_name}_{name}.wav")
...
# Voice sound themes live in sidecar/sounds/<theme>/start.mp3, stop.mp3
...
def play_sound(name: str):
    ...
    if SOUND_THEME == "silent":
        return

    path = None
    if SOUND_THEME in _VOICE_THEMES:
        # Voice themes have start.wav + stop.wav only.
        # Map all sound events to the appropriate voice file.
        voice_map = {
            "start": "start", "activate": "start",
            "stop": "stop", "done": "stop", "command": "stop", "error": "stop",
        }
        mapped = voice_map.get(name, name)
        voice_file = _SIDECAR_DIR / "sounds" / SOUND_THEME / f"{mapped}.wav"
        if voice_file.is_file():
            path = str(voice_file)
        else:
            path = os.path.join(SOUNDDIR, f"subtle_{name}.wav")
    else:
        path = os.path.join(SOUNDDIR, f"{SOUND_THEME}_{name}.wav")
        if not os.path.isfile(path):
            path = os.path.join(SOUNDDIR, f"subtle_{name}.wav")
...
def run_evdev_listener():
    ...
    trigger_code = getattr(ecodes, TRIGGER_KEY, ecodes.KEY_RIGHTCTRL)
    ...
    logger.info("Hold %s to talk. Short = command, long = dictation.", TRIGGER_KEY)
    ...
    try:
        for event in dev.read_loop():
            if event.type != ecodes.EV_KEY:
                continue
            # Re-resolve trigger code each event so runtime changes work
            current_code = getattr(ecodes, TRIGGER_KEY, ecodes.KEY_RIGHTCTRL)
...
def run_pynput_listener():
    ...
    # Mapping from evdev-style names to pynput keys
    _pynput_map = {
    ...
    def _resolve_trigger():
        return _pynput_map.get(TRIGGER_KEY)

    trigger_key = _resolve_trigger()
    if trigger_key is None:
        logger.error("Unknown trigger key: %s", TRIGGER_KEY)
        sys.exit(1)
...
def run_vad_listener():
    ...
    oww_threshold = WAKEWORD_THRESHOLD
...
def _load_flutter_settings():
    """Read ~/.config/talkye/settings.json to pick up Flutter-saved settings."""
    global INPUT_MODE, TRIGGER_KEY, SOUND_THEME, MAGIC_WORD, VAD_ACTIVE_TIMEOUT, VAD_AUTO_ENTER, WAKEWORD_THRESHOLD
    try:
        import json
        ...
        if os.path.isfile(settings_path):
            ...
            if "inputMode" in cfg:
                ...
                logger.info("Settings: input_mode=%s (from settings.json)", INPUT_MODE)
            if "triggerKey" in cfg:
                TRIGGER_KEY = cfg["triggerKey"]
                logger.info("Settings: trigger_key=%s (from settings.json)", TRIGGER_KEY)
            if "soundTheme" in cfg:
                SOUND_THEME = cfg["soundTheme"]
                logger.info("Settings: sound_theme=%s (from settings.json)", SOUND_THEME)
            if "magicWord" in cfg:
                MAGIC_WORD = cfg["magicWord"].lower()
                logger.info("Settings: magic_word=%s (from settings.json)", MAGIC_WORD)
            if "vadTimeout" in cfg:
                VAD_ACTIVE_TIMEOUT = int(cfg["vadTimeout"])
                logger.info("Settings: vad_timeout=%ds (from settings.json)", VAD_ACTIVE_TIMEOUT)
            if "autoEnter" in cfg:
                VAD_AUTO_ENTER = bool(cfg["autoEnter"])
                logger.info("Settings: auto_enter=%s (from settings.json)", VAD_AUTO_ENTER)
            if "wakewordThreshold" in cfg:
                WAKEWORD_THRESHOLD = float(cfg["wakewordThreshold"])
                logger.info("Settings: wakeword_threshold=%.2f (from settings.json)", WAKEWORD_THRESHOLD)
    except Exception as e:
        logger.warning("Failed to load Flutter settings: %s", e)


def main():
    # Load Flutter-saved settings before anything else
    _load_flutter_settings()
    ...
    if INPUT_MODE == "vad":
        ...
        try:
            ...
            # Test if we can actually access input devices (need group 'input')
