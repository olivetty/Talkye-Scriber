...
Future<void> performUpdate(
  ...
  // Use a helper script to restart — this ensures the old process (and its
  // FUSE mount) is fully dead before the new AppImage launches.
  // Without this, the kernel may serve the old AppImage from page cache.
  final restartScript = '$home/.config/talkye/restart.sh';
  File(restartScript).writeAsStringSync(
    '#!/bin/bash\n'
    '# Auto-generated by Talkye updater — restarts after update\n'
    'sleep 2\n'
    'exec "\$1"\n',
  );
  await Process.run('chmod', ['+x', restartScript]);

  // Launch the restart script detached — it waits for us to die, then starts new
  await Process.start(restartScript, [
    appImagePath,
  ], mode: ProcessStartMode.detached);

  // Kill sidecar so it doesn't hold the port
  try {
    await Process.run('pkill', ['-f', 'uvicorn.*server:app.*8179']);
  } catch (_) {}

  // Exit immediately — the script will start the new version after we're gone
  exit(0);
}
